    ...
    while (std::getline(inputFile, line)) {
        ...
        for (const auto& g : games) {
            ...
            for (const auto& c : g.combinations) {
                if (requirement.combinations.at(0).red < c.red || 
                    requirement.combinations.at(0).green < c.green ||
                    requirement.combinations.at(0).blue < c.blue) {
                        gameWon = false;
                        break;
                    }
            }
            if (gameWon) {
                winningGamesSum += g.id;
            }
        }

        std::cout << "Winning games: " << winningGamesSum << std::endl;
    }
    inputFile.close();
}